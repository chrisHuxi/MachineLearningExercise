...
def calcIgRatio(examples, attr_index, cls_index):
    sub_examples_dict = {}
    for example in examples:
        if example[attr_index] in set(sub_examples_dict.keys()):
            sub_examples_dict[example[attr_index]].append(example)
    ...
    ig = calc_entropy(examples,cls_index) - remainder
    
    iv = 0.0
    for Tv in sub_examples_dict.keys():
        iv += - len(sub_examples_dict[Tv])/(len(examples))*log(len(sub_examples_dict[Tv])/(len(examples)),2)
    igr = ig/iv
    return igr
...
def choose_best_attr(examples, attr_avail, cls_index):
    ...
    for i in range(cls_index):
        if attr_avail[i] == True:
            igs.append(calcIgRatio(examples, i, cls_index))
        else:
            igs.append(-1.0)
    #print(igs)
...
def findThresholdFast(numeric_list,examples,attr_index,cls_index):
    ...
    for num in sorted_list:
        if attr_split_dict[num][0] == temple_cls:
            continue
        elif attr_split_dict[num][0] != temple_cls and accumulative <= 1: #累计数量不够   
            accumulative += attr_split_dict[num][1]
        elif attr_split_dict[num][0] != temple_cls and accumulative > 1:
...
if __name__ == '__main__':
    ...
    examples,test_examples = openfile() # load the training set
    examples,attr,threshold_dict = numericValueProcessTraining(examples)
    test, test_attr = numericValueProcessTest(test_examples,threshold_dict)
