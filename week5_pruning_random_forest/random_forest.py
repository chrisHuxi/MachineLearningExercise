...
def selectFeaturesRandom(attr_avail):
    feature_index = np.nonzero(attr_avail)[0]

    feature_number = int(np.sqrt(len(feature_index)))+3
    #I set selected feature_number as 5
    feature_index_random = random.sample(feature_index.tolist(),feature_number)
    new_attr_avail = [0]*len(attr_avail)
    for i in feature_index_random:
        new_attr_avail[i] = 1
    return new_attr_avail

    
def dforest_learning(tree_number, examples, attr_avail, default, cls_index):
    dforest = []
    for i in range(tree_number):
        new_attr_avail = selectFeaturesRandom(attr_avail)
        dtree = dtree_learning(examples, new_attr_avail, default, cls_index)
        dforest.append(dtree)
    return dforest
    
def dforest_classify(dforest, x):
    result = {}
    for every_tree in dforest:
        if dtree_classify(every_tree, x) not in set(result.keys()):
            result[dtree_classify(every_tree, x)] = 1
        else:
            result[dtree_classify(every_tree, x)] += 1
            
    return max(result, key=lambda x: result[x])

def dforest_test(dforest, examples, cls_index):    
    """Classify all examples using the given decision tree. Prints the achieved accuracy."""
    
    correct = 0
    for example in examples:
        if dforest_classify(dforest,example)==example[cls_index]:
            correct += 1
...
training = examples[0:int(len(examples)*0.9)]
...
tree_number = 50
dforest = dforest_learning(tree_number, training, attr_avail, majority(training,cls_index), cls_index)

dforest_test(dforest, examples, cls_index)
dforest_test(dforest, test, cls_index)
